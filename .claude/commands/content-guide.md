...
Permite saber **de onde vieram os leads** — influencers, anúncios, e-mails, bio do Instagram, etc. — sem nenhuma alteração no código do site.

O GA4 lê automaticamente os parâmetros UTM da URL e associa **toda a sessão e todos os eventos** àquela origem. Isso inclui os eventos de funil (`select_item` ao clicar num serviço, `view_item` ao abrir a página de detalhe e `generate_lead` ao clicar no CTA).

### Como funciona

Você entrega ao influencer (ou usa no anúncio) um link assim:

```
https://seusite.com.br?utm_source=instagram&utm_medium=influencer&utm_campaign=nome-influencer
```

Quando alguém acessa via esse link, o GA4 registra a origem e vincula todos os eventos daquela visita à campanha.

### Parâmetros disponíveis

| Parâmetro | Obrigatório | Exemplo | O que representa |
|---|---|---|---|
| `utm_source` | sim | `instagram`, `google`, `email` | De onde vem o tráfego |
| `utm_medium` | sim | `influencer`, `cpc`, `social` | Tipo de canal |
| `utm_campaign` | sim | `joao-silva-abril`, `lancamento-2026` | Campanha específica |
| `utm_content` | não | `stories`, `reels`, `bio` | Formato ou criativo (útil para A/B) |
| `utm_term` | não | `palavra-chave` | Para anúncios de busca paga |

### Como gerar os links

Use o **[Campaign URL Builder](https://ga-dev-tools.google/campaign-url-builder/)** do Google — preenche os campos e ele monta a URL corretamente.

Para encurtar o link antes de passar para o influencer, use o [Bitly](https://bitly.com) ou o encurtador nativo do Instagram.

### Onde ver os dados no GA4

**Aquisição → Aquisição de tráfego** — filtre por `Campanha` para comparar lado a lado a performance de cada influencer/anúncio.

Para ver o funil completo de uma campanha específica:
1. Acesse **Explorar → Funil**
2. Adicione os eventos: `view_item` → `generate_lead`
3. Aplique o filtro `utm_campaign = nome-da-campanha`

Você verá quantas pessoas chegaram pelo link do influencer, quantas visualizaram um serviço e quantas clicaram no CTA de contato.

> Não é necessário alterar nenhum arquivo do site para usar UTMs. Os links são criados externamente e funcionam com o GA4 já configurado em `content/analytics.json`.
